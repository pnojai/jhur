rankhospital <- function(state, outcome, num = "best") {
        ...
        # order() is a vector, so you can pick off the index you want.
        # Check this out.
        hospital_ordered <- order(hospitals[2], hospitals[1])
        if (num == "best") {
                hospital_rank <- hospital_ordered[1]
        } else if (num == "worst") {
                hospital_rank <- hospital_ordered[length(hospital_ordered)]
        } else {
                hospital_rank <- hospital_ordered[num]
        }
        
        hospitals[hospital_rank, 1]
